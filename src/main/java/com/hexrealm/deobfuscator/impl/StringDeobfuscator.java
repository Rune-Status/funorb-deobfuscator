...
@Log4j2
public class StringDeobfuscator implements Deobfuscator {
        ...
        classNodes.forEach(classNode -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (next.getOpcode() == Opcodes.CALOAD) {
                    stringKey = extractIntValue(next.getNext());
                    break;
                }
            }
            ...
            if (tableSwitchNode != null) {
                ...
                for (int i = 0; i < labels.size(); i++) {
                    AbstractInsnNode abstractInsnNode = labels.get(i).getNext().getNext();

                    keys[i] = (byte) extractIntValue(abstractInsnNode);
                }
                keys[4] = (byte) extractIntValue(tableSwitchNode.dflt.getNext().getNext());
    ...
    private static int extractIntValue(AbstractInsnNode abstractInsnNode) {
